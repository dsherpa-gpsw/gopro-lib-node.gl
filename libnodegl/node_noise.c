...
static const struct node_param noise_params[] = {
    {"frequency",   PARAM_TYPE_DBL, OFFSET(frequency), {.dbl=1.},
                    .flags=PARAM_FLAG_ALLOW_LIVE_CHANGE,
                    .desc=NGLI_DOCSTRING("oscillation per second")},
    {"amplitude",   PARAM_TYPE_DBL, OFFSET(generator_params.amplitude), {.dbl=1.},
                    .flags=PARAM_FLAG_ALLOW_LIVE_CHANGE,
                    .desc=NGLI_DOCSTRING("by how much it oscillates")},
    {"octaves",     PARAM_TYPE_INT, OFFSET(generator_params.octaves), {.i64=3},
                    .flags=PARAM_FLAG_ALLOW_LIVE_CHANGE,
                    .desc=NGLI_DOCSTRING("number of accumulated noise layers (controls the level of details)")},
    {"lacunarity",  PARAM_TYPE_DBL, OFFSET(generator_params.lacunarity), {.dbl=2.0},
                    .flags=PARAM_FLAG_ALLOW_LIVE_CHANGE,
                    .desc=NGLI_DOCSTRING("frequency multiplier per octave")},
    {"gain",        PARAM_TYPE_DBL, OFFSET(generator_params.gain), {.dbl=0.5},
                    .flags=PARAM_FLAG_ALLOW_LIVE_CHANGE,
                    .desc=NGLI_DOCSTRING("amplitude multiplier per octave (also known as persistence)")},
    {"seed",        PARAM_TYPE_UINT, OFFSET(generator_params.seed), {.i64=0},
                    .desc=NGLI_DOCSTRING("random base seed (acts as an offsetting to the time)")},
    {"interpolant", PARAM_TYPE_SELECT, OFFSET(generator_params.function), {.i64=NGLI_NOISE_QUINTIC},
...
static int init_noise_generators(struct noise_priv *s, int n)
{
     ...
     */
    const uint32_t seed_offset = 0xffffffff / n;
