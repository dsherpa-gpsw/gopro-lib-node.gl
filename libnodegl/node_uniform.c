...
static int uniform##name##_update_func(struct ngl_node *node, va_list ap) \
{                                                                         \
...
static int uniformint_update_func(struct ngl_node *node, va_list ap) // NEEDED BECAUSE WE CANT va_arg(ap, void *) with a single int params
{
    int ret = set_live_changed(node);
    if (ret < 0)
        return ret;
...
static int uniformfloat_update_func(struct ngl_node *node, va_list ap)
{
    int ret = set_live_changed(node);
    if (ret < 0)
        return ret;
...
static int uniformquat_update_func(struct ngl_node *node, va_list ap)
{
    int ret = set_live_changed(node);
    if (ret < 0)
        return ret;
    ...
    struct variable_priv *s = node->priv_data;
    memcpy(s->opt.vec, value, s->data_size);
