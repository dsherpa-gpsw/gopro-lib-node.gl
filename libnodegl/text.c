...
static enum char_category get_codepoint_category(unsigned codepoint)
{
#if 0
    // XXX: doesn't work: we get glyph indexes since we are post shaping...
    // XXX: also there might not be a linebreak symbol?
    if (codepoint == ' ')
        return NGLI_TEXT_CHAR_CATEGORY_SPACE;
    if (codepoint == '\n')
        return NGLI_TEXT_CHAR_CATEGORY_LINEBREAK;
#endif
    return NGLI_TEXT_CHAR_CATEGORY_NONE;
}
    ...
    for (int i = 0; i < ngli_darray_count(&s->lines); i++) {
        ...
        for (int cp = 0; cp < len; cp++) {
            ...
            struct char_info chr = {
                ...
                .w = glyph->w,
                .h = glyph->h,
                .category = get_codepoint_category(glyph_id),
            };
            //LOG(ERROR, "%u -> %d", glyph_id, chr.category);
        ...
        if (i != ngli_darray_count(&s->lines)) {
            struct char_info chr = {
                .category = NGLI_TEXT_CHAR_CATEGORY_LINEBREAK,
            };
            if (!ngli_darray_push(&s->chars, &chr))
                return NGL_ERROR_MEMORY;
        }
...
int ngli_text_set_string(struct text *s, const char *str)
{
    ...
    for (int i = 0; str[i]; i++) {
        ...
        struct char_info chr = {
            ...
            .w = NGLI_FONT_W,
            .h = NGLI_FONT_H,
            .category = get_codepoint_category(str[i]),
        };
