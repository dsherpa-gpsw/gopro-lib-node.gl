...
static int update_character_geometries(struct ngl_node *node)
{
    ...
    if (nb_indices > s->nb_indices) { // need re-alloc
        ...
        if ((ret = ngli_buffer_init(s->vertices, nb_vertices * sizeof(*vertices), VERTEX_USAGE_FLAGS)) < 0 ||
            ...
            (ret = ngli_buffer_init(s->glyph_rects, nb_glyph_rects * sizeof(*glyph_rects), VERTEX_USAGE_FLAGS)) < 0 ||
            (ret = ngli_buffer_init(s->glyph_sizes, nb_glyph_sizes * sizeof(*glyph_sizes), VERTEX_USAGE_FLAGS)) < 0)
            goto end;
